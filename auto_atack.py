    ...
    try:
        ...
        if j > 0:
            ...
            if mana >= min_mana_atack:
                if spell_um_nm!= '':
                    while (True):
                        j1 = pyautogui.locateOnScreen('magias/'+spell_um_nm+'.png')
                        if j1 != None:
                            cast_spell(spell_um_nm,spell_um_ht)
                            break
                    if spell_dois_nm != '':
                        while (True):
                            j2 = pyautogui.locateOnScreen('magias/'+spell_dois_nm+'.png')
                            if j2 != None:
                                cast_spell(spell_dois_nm, spell_dois_ht)
                                break
                        if spell_tres_nm != '':
                            while (True):
                                j3 = pyautogui.locateOnScreen('magias/'+spell_tres_nm+'.png')
                                if j3 != None:
